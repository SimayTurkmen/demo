...
public class SunriseSunsetCalculator {
    ...
    public double getDayLength(String setHour, String riseHour){
        double setHourNum = Double.parseDouble(setHour.substring(0,2));
        double riseHourNum = Double.parseDouble(riseHour.substring(0,2));
        double diffHour = setHourNum - riseHourNum; //11 saat
        if(diffHour<0) { diffHour = diffHour + 24.0; }
        double setMinNum = Double.parseDouble(setHour.substring(3));
        double riseMinNum = Double.parseDouble(riseHour.substring(3));
        double diffMin = setMinNum - riseMinNum;
        if(diffMin>0){
            return diffHour + diffMin/100;
        }
        else{
            return (diffHour-1) + (0.60-Math.abs(diffMin/100));
        }
    }
